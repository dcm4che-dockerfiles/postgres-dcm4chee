...
create table queue_msg (pk int8 not null, batchID varchar(255), created_time timestamp not null, device_name varchar(255) not null, error_msg varchar(255), msg_body bytea not null, msg_id varchar(255) not null, msg_props varchar(4000) not null, num_failures int4 not null, outcome_msg varchar(255), priority int4 not null, proc_end_time timestamp, proc_start_time timestamp, queue_name varchar(255) not null, scheduled_time timestamp not null, msg_status int4 not null, updated_time timestamp not null, version int8, primary key (pk));
...
create table stgcmt_result (pk int8 not null, batch_id varchar(255), created_time timestamp not null, device_name varchar(255) not null, exporter_id varchar(255) not null, msg_id varchar(255), num_failures int4, num_instances int4, series_iuid varchar(255), sop_iuid varchar(255), stgcmt_status int4 not null, study_iuid varchar(255) not null, transaction_uid varchar(255) not null, updated_time timestamp not null, primary key (pk));
